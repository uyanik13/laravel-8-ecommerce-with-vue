  ...
  <div id="data-list-list-view" class="data-list-container">
    ...
    <div id="AllList" class="data-list-container">
      <vs-table ref="AllList"  multiple v-model="selected" pagination :max-items="itemsPerPage" search :data="data">
        ...
        <template slot="thead">
          <vs-th sort-key="img">{{$t('Image')}}</vs-th>
        ...
        <template slot-scope="{data}">
          ...
          <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
            <vs-td class="img-container">
              <img :src="tr.user.avatar" class="product-img"   height="50px" width="50px"/>
            </vs-td>
            <vs-td>
            ...
            <vs-td>
              <p class="product-name font-medium truncate">{{ $t(JSON.parse(tr.options).profession) }}</p>
            </vs-td>
            <vs-td>
              <p class="product-name font-medium truncate">${{ JSON.parse(tr.options).price }}</p>
            </vs-td>

            <vs-td>
              <vs-chip :color="getDateColor(tr.expires_date)" class="product-order-status">{{ toDate(tr.expires_date)}}</vs-chip>
            </vs-td>
            <!--              <vs-td>-->
            ...
            <vs-td>
              <vs-chip :color="getOrderStatusColor(tr.status)" class="product-order-status">{{ tr.status == '1' ? $t('Active') : $t('Inactive')}}</vs-chip>
            </vs-td>


            <vs-td class="whitespace-no-wrap">
              <popup-background :activeUser="user" :ServiceData="tr"/>
              <feather-icon icon="EditIcon" v-show="tr.user_id === user.id" svgClasses="w-7 h-7 hover:text-primary stroke-current" @click="editData(tr)" />
            </vs-td>
