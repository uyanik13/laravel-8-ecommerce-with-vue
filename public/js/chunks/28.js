  ...
  methods: {
    checkLogin: function checkLogin() {
      ...
      if (this.$acl.check('user')) {
        this.showAlert(_i18n_i18n__WEBPACK_IMPORTED_MODULE_0__["default"].t('Error'), _i18n_i18n__WEBPACK_IMPORTED_MODULE_0__["default"].t('you_are_logged_in'), 'icon-alert-circle', 'warning');
    ...
    registerUserJWt: function registerUserJWt() {
      ...
      this.$store.dispatch('auth/registerUserJWT', payload).then(function (response) {
        ...
        _this.showAlert(_i18n_i18n__WEBPACK_IMPORTED_MODULE_0__["default"].t('Success'), _i18n_i18n__WEBPACK_IMPORTED_MODULE_0__["default"].t('yourAccountCreated'), 'icon-success', 'success'); //console.log('rSTATUS', response.data.status)


        if (response.data.status === 'verification.sent') _this.$router.push({
          name: 'login'
        });
